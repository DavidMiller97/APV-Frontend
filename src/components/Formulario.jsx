...
const Formulario = () => {
    ...
    useEffect(() => {
        ...
        if(paciente?.propietario) setPropietario(paciente.propietario);
        if(paciente?.email) setEmail(paciente.email);
        if(paciente?.fecha) setFecha(paciente.fecha);
        if(paciente?.sintomas) setSintomas(paciente.sintomas);
        setId(paciente._id);
        ...
        <>
            ...
            <form className="bg-white py-10 px-5 mb-10 lg:mb-0 shadow-md rounded-md" onSubmit={handleSubmit}>
                <div className="mb-5">
                    ...
                    <input type="text" id="mascota" placeholder="Nombre Mascota" className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md" value={nombre} onChange={(e) => setNombre(e.target.value)}/>
                </div>
                <div className="mb-5">
                ...
                <input type="submit" value={!paciente._id ? 'Agregar paciente' : 'Editar paciente'} className="bg-indigo-600 w-full p-3 text-white uppercase font-bold hover:bg-indigo-700 cursor-pointer transition-colors"/>
                <input type="button" value="Limpiar" className="bg-gray-600 w-full p-3 text-white uppercase font-bold hover:bg-indigo-700 cursor-pointer transition-colors" onClick={() => limpiarFormulario()}/>
            </form>
        </>
